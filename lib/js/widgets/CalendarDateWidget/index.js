  ...
  _createClass(CalendarDateWidget, [{
    ...
    value: function handleFormChange(field, value) {
      ...
      if ((field === 'day' && !!this.state.value.month || field === 'month' && !!this.state.value.day) && !this.state.value.year) {
        var isDateFromLastYear = (0, _dateFns.isAfter)(new Date(currentYear, newState.value.month - 1, newState.value.day), new Date(new Date()));
        var autofillYear = isDateFromLastYear ? currentYear - 1 : currentYear;
      ...
      if (!!newState.value.year && !!newState.value.day && !!newState.value.month) {
        newState = (0, _dateHelpers.setValidDate)(newState);
      }
